...
public class OnepunchCmd extends Cmd{
	public OnepunchCmd(){
		super("onepunch");
		addString("no-player", "&7Only players can execute this command.");
		addString("no-permission", "&7You do not have permission to run this command.");
		addString("no-online", "&7This player is not online.");
		addString("enabled", "&7Your &aonepunch &7has been enabled.");
		addString("disabled", "&7Your &aonepunch &7has been disabled.");
		addString("enabled-to", "&7The &aonepunch &7of the player &c{PLAYER} &7has been enabled.");
		addString("disabled-to", "&7The &aonepunch &7of the player &c{PLAYER} &7has been disabled.");
	}
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args){
		if (sender instanceof Player){
			Player p = (Player) sender;
			if (PermissionsUtils.hasPermission(p, "onepunch")){
				try{
					...
					if (!DirectMC.hasOnepunch(p)){
						DirectMC.getOnepunches().add(p);
						DirectMC.sendMessage(p, getString("enabled"));
						return true;
					}else{
						DirectMC.getOnepunches().remove(p);
